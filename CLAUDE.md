...
**CORS is wired through Spring Security, not `@CrossOrigin`.** `CorsConfig` builds the `CorsConfigurationSource` bean from `app.cors.allowed-origins`; `SecurityConfig` injects that bean and registers it via `http.cors(...)`.

**Stateless JWT auth, not session-based.** `POST /api/auth/login` (`AuthController` → `AuthService`) checks a username/BCrypt-hashed-password pair against the `users` table and returns a signed JWT (`JwtService`, `io.jsonwebtoken`, HMAC-SHA256, 24h expiry) carrying the username as subject and `fullName` as a claim. `JwtAuthenticationFilter` (a `OncePerRequestFilter` registered before `UsernamePasswordAuthenticationFilter`) reads the `Authorization: Bearer` header on every request, validates the token, and populates `SecurityContextHolder` — there's no session, no `UserDetailsService`, and the filter grants no authorities (the API has one protected tier, not roles). `SecurityConfig` permits `/api/auth/**` and the springdoc/swagger paths, and requires a valid token for everything else (`/api/credits/**`). Seed users (`V5__seed_users.sql`) map 1:1 to the sales agent names already present in the credits seed data, so a logged-in user's `fullName` can be used to auto-fill "Asesor de ventas" on the frontend.
